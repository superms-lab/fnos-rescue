...
class LiveProfileTests(unittest.TestCase):
    def test_profile_has_recovery_tools_and_local_only_web(self):
        ...
        self.assertIn("OVMF_VARS", smoke)

        bios = (ROOT / "live/config/bootloaders/isolinux/isolinux.cfg").read_text()
        bios_live = (ROOT / "live/config/bootloaders/isolinux/live.cfg").read_text()
        self.assertIn("default live-amd64", bios)
        self.assertNotIn("vesamenu.c32", bios)
        self.assertIn("kernel /live/vmlinuz", bios_live)
        self.assertIn("console=ttyS0,115200n8", bios_live)
        self.assertNotIn("@KERNEL@", bios_live)
        bios_template = (ROOT / "live/config/bootloaders/isolinux/live.cfg.in").read_text()
        self.assertNotIn("@LINUX@", bios_template)
        self.assertNotIn("@APPEND_LIVE@", bios_template)
