...
> 当前进度：核心恢复链路、fnOS Web 服务和发布加固已完成；Live ISO 已在 Linux CI 中通过 BIOS 与 UEFI 虚拟机完整启动验收。
...
- 提供仅监听本机的引导式 Web 恢复控制台和沙箱化 fnOS 服务。
- 提供经 BIOS/UEFI 启动验收的 Live ISO，供 Windows、macOS、Linux 和 NAS 主机使用。
...
tar -xzf dist/fnos-rescue_0.1.1_fnos_x86_64.tar.gz
sudo ./fnos-rescue_0.1.1_x86_64/install.sh
