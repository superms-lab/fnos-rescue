			    ...
			    struct btrfs_key *key, const char *file)
{
	...
	while (1) {
		...
		if (getenv("BTRFS_DUMP_FILE_EXTENTS")) {
			fprintf(stderr,
				"FILE_EXTENT inode=%llu file_offset=%llu type=%d compression=%d disk_bytenr=%llu disk_num_bytes=%llu extent_offset=%llu num_bytes=%llu ram_bytes=%llu\n",
				(unsigned long long)found_key.objectid,
				(unsigned long long)found_key.offset,
				extent_type, compression,
				(unsigned long long)btrfs_file_extent_disk_bytenr(leaf, fi),
				(unsigned long long)btrfs_file_extent_disk_num_bytes(leaf, fi),
				(unsigned long long)btrfs_file_extent_offset(leaf, fi),
				(unsigned long long)btrfs_file_extent_num_bytes(leaf, fi),
				(unsigned long long)btrfs_file_extent_ram_bytes(leaf, fi));
		}
		      ...
		      const regex_t *mreg)
{
	...
	while (leaf) {
		...
		if (type == BTRFS_FT_REG_FILE) {
			...
			ret = btrfs_restore_copy_file(root, fd, &location, path_name);
			close(fd);
			if (ret) {
				 ...
				 const char *in_dir)
{
	...
	/* Recovery mode: preserve every readable file and continue past damage. */
	ignore_errors = 1;
	overwrite = 1;
	restore_symlinks = 1;
	get_snaps = 1;
	dry_run = 0;
	return search_dir(root, &key, output_rootdir, in_dir, NULL);
}
