...
int btrfs_restore_copy_file(struct btrfs_root *root, int fd,
			    struct btrfs_key *key, const char *file);
int btrfs_restore_cached_subtree(struct btrfs_root *root, u64 dirid,
				 const char *output_rootdir,
				 const char *in_dir);
...
static u32 count_chunk_list(struct list_head *head)
{
...
static int write_chunk_list(FILE *f, struct list_head *head, u32 kind)
{
	struct chunk_record *chunk;
	struct chunk_cache_record out;

	list_for_each_entry(chunk, head, list) {
		memset(&out, 0, sizeof(out));
		out.list_kind = kind;
		out.num_stripes = chunk->num_stripes;
		out.sub_stripes = chunk->sub_stripes;
		out.generation = chunk->generation;
		out.objectid = chunk->objectid;
		out.type = chunk->type;
		out.unaligned = chunk->unaligned;
		out.offset = chunk->offset;
		out.owner = chunk->owner;
		out.length = chunk->length;
		out.type_flags = chunk->type_flags;
		out.stripe_len = chunk->stripe_len;
		out.io_align = chunk->io_align;
		out.io_width = chunk->io_width;
		out.sector_size = chunk->sector_size;
		if (fwrite(&out, sizeof(out), 1, f) != 1 ||
		    fwrite(chunk->stripes, sizeof(struct stripe),
			   chunk->num_stripes, f) != chunk->num_stripes)
			return -EIO;
	}
	return 0;
}

static int save_chunk_cache(struct recover_control *rc, const char *path)
{
	struct chunk_cache_header hdr = { .magic = CHUNK_CACHE_MAGIC, .version = 1 };
	FILE *f;
	int ret;

	hdr.count = count_chunk_list(&rc->good_chunks) +
		    count_chunk_list(&rc->rebuild_chunks) +
		    count_chunk_list(&rc->unrepaired_chunks) +
		    count_chunk_list(&rc->bad_chunks);
	f = fopen(path, "wb");
	if (!f)
		return -errno;
	ret = fwrite(&hdr, sizeof(hdr), 1, f) == 1 ? 0 : -EIO;
	if (!ret)
		ret = write_chunk_list(f, &rc->good_chunks, 1);
	if (!ret)
		ret = write_chunk_list(f, &rc->rebuild_chunks, 2);
	if (!ret)
	...
	if (fclose(f) && !ret)
		ret = -errno;
	if (!ret)
		fprintf(stderr, "Saved %u chunk mappings to %s\n", hdr.count, path);
	return ret;
}

static int load_chunk_cache(struct recover_control *rc, const char *path)
{
	struct chunk_cache_header hdr;
	struct chunk_cache_record in;
	struct chunk_record *chunk;
	...
	for (i = 0; i < hdr.count; i++) {
		...
		chunk->generation = in.generation;
		chunk->objectid = in.objectid;
		chunk->type = in.type;
		chunk->offset = in.offset;
		chunk->owner = in.owner;
		chunk->length = in.length;
		chunk->type_flags = in.type_flags;
		chunk->stripe_len = in.stripe_len;
		chunk->num_stripes = in.num_stripes;
		chunk->sub_stripes = in.sub_stripes;
		chunk->io_align = in.io_align;
		chunk->io_width = in.io_width;
		chunk->sector_size = in.sector_size;
		chunk->unaligned = in.unaligned;
		chunk->cache.start = in.offset;
		chunk->cache.size = in.length;
		if (fread(chunk->stripes, sizeof(struct stripe),
			  in.num_stripes, f) != in.num_stripes) {
			free(chunk);
			ret = -EIO;
			goto out;
		}
		...
		switch (in.list_kind) {
		case 1: head = &rc->good_chunks; break;
		case 2: head = &rc->rebuild_chunks; break;
		case 3: head = &rc->unrepaired_chunks; break;
		default: head = &rc->bad_chunks; break;
		}
...
static int list_cached_dir(struct btrfs_root *root, u64 dirid,
			   const char *prefix, FILE *out, int depth)
{
	struct btrfs_path path = { 0 };
	...
	while (path.nodes[0]) {
		...
		btrfs_item_key_to_cpu(leaf, &found, path.slots[0]);
		if (found.objectid != dirid || found.type != BTRFS_DIR_INDEX_KEY)
			break;
		...
		if (type == BTRFS_FT_DIR) {
			...
			if (location.type == BTRFS_ROOT_ITEM_KEY) {
				...
				child_root = btrfs_read_fs_root(root->fs_info, &location);
...
open_ctree_with_broken_chunk(struct recover_control *rc)
{
	...
	/*
	 * Recovery can still rebuild the chunk tree when a non-essential global
	 * tree (for example an old checksum root) lives in an unrecoverable chunk.
	 * Use a partial open so readable historical root and filesystem trees can
	 * be used while missing global roots get synthetic placeholders.  This
	 * binary is used only against a disposable QCOW recovery overlay.
	 */
	ret = btrfs_setup_all_roots(fs_info, 0, OPEN_CTREE_PARTIAL);
	if (ret)
		goto out_failed;
...
int btrfs_recover_chunk_tree(const char *path, int yes)
{
	...
	struct recover_control rc;
	const char *cache_load = getenv("BTRFS_CHUNK_CACHE_LOAD");
	const char *cache_save = getenv("BTRFS_CHUNK_CACHE_SAVE");
	...
	if (cache_load) {
		ret = load_chunk_cache(&rc, cache_load);
		if (ret) {
			fprintf(stderr, "load chunk mapping cache failed: %d\n", ret);
			goto fail_rc;
		}
		goto mappings_ready;
	}
	...
	print_check_result(&rc);
	if (cache_save) {
		ret = save_chunk_cache(&rc, cache_save);
		if (ret) {
			fprintf(stderr, "save chunk mapping cache failed: %d\n", ret);
			goto fail_rc;
		}
		if (getenv("BTRFS_CHUNK_CACHE_ONLY"))
			goto fail_rc;
	}
...
mappings_ready:
	...
	if (getenv("BTRFS_FORCE_FS_ROOT")) {
		...
		u64 fs_bytenr = strtoull(getenv("BTRFS_FORCE_FS_ROOT"), NULL, 0);
		struct extent_buffer *forced;

		forced = read_tree_block(root->fs_info, fs_bytenr, &check);
		if (!extent_buffer_uptodate(forced)) {
			fprintf(stderr, "failed to read forced fs root %llu\n",
		...
		fprintf(stderr, "Using forced historical fs root %llu generation %llu\n",
			(unsigned long long)fs_bytenr,
			(unsigned long long)btrfs_header_generation(forced));
	}
	if (getenv("BTRFS_LIST_PATHS")) {
		FILE *out = fopen(getenv("BTRFS_LIST_PATHS"), "w");
	...
	if (getenv("BTRFS_EXTRACT_INODE") && getenv("BTRFS_EXTRACT_PATH")) {
		struct btrfs_root *extract_root = root;
		struct btrfs_key file_key = {
			.objectid = strtoull(getenv("BTRFS_EXTRACT_INODE"), NULL, 0),
			.type = BTRFS_INODE_ITEM_KEY,
			.offset = 0,
		};
		const char *rootid_env = getenv("BTRFS_EXTRACT_ROOTID");
		const char *output = getenv("BTRFS_EXTRACT_PATH");
		int fd;

		if (rootid_env) {
			u64 rootid = strtoull(rootid_env, NULL, 0);
			if (rootid != root->root_key.objectid) {
				...
				extract_root = btrfs_read_fs_root(root->fs_info, &root_key);
				if (IS_ERR(extract_root)) {
					ret = PTR_ERR(extract_root);
					goto fail_close_ctree;
				}
			}
		}
		if (getenv("BTRFS_FORCE_EXTRACT_ROOT")) {
			struct btrfs_tree_parent_check check = { 0 };
			u64 bytenr = strtoull(getenv("BTRFS_FORCE_EXTRACT_ROOT"),
						NULL, 0);
			struct extent_buffer *forced;

			forced = read_tree_block(root->fs_info, bytenr, &check);
			if (!extent_buffer_uptodate(forced)) {
				fprintf(stderr,
					"failed to read forced extract root %llu\n",
			...
			fprintf(stderr,
				"Using forced historical extract root %llu generation %llu owner %llu\n",
				(unsigned long long)bytenr,
				(unsigned long long)btrfs_header_generation(forced),
				(unsigned long long)btrfs_header_owner(forced));
		}
	...
	if (getenv("BTRFS_RESTORE_DIRID") && getenv("BTRFS_RESTORE_OUTPUT") &&
	    getenv("BTRFS_RESTORE_PREFIX")) {
		u64 dirid = strtoull(getenv("BTRFS_RESTORE_DIRID"), NULL, 0);

		ret = btrfs_restore_cached_subtree(root, dirid,
				getenv("BTRFS_RESTORE_OUTPUT"),
				getenv("BTRFS_RESTORE_PREFIX"));
		goto fail_close_ctree;
	}
